...
local sounds = {
    ...
    Execute = "rbxassetid://6895079853",  -- Som de execução
    ...
    if isfolder(pasta) then
        ...
        if success and files then
            for _, filePath in ipairs(files) do
                -- Verificar se é um arquivo .png (logo) e NÃO é o favorites.json
...
-- "keys": ["key1", "key2", "key3"]
-- DADOS DOS JOGOS (Estrutura atualizada para suportar múltiplas chaves)
-- DADOS DOS JOGOS
local gamesDataRaw = [[
[
    {
        "categoria": "JOGOS ALEATÓRIO",
        "jogos": [
            {
            ...
            {
                "titulo": "Corrida de avião com 99 hélices",
            ...
            {
                "titulo": "Minha coleção de cartões",
...
searchBox.PlaceholderText = "🔍 Buscar jogos..."
searchBox.Text = ""
...
local function createGameCard(parent, gameData)
    ...
    -- Botão de Estrela (Favorito) - ZIndex maior para evitar conflito
    local starBtn = Instance.new("TextButton")
    starBtn.Size = UDim2.new(0, 25, 0, 25)
    starBtn.Position = UDim2.new(1, -28, 0, 3)
    starBtn.BackgroundTransparency = 1
    starBtn.Text = favorites[gameData.titulo] and "⭐" or "☆"
    ...
    -- Botão de Execução (Fica atrás da estrela)
...
-- Tentar conectar ao evento de fechamento (se disponível)
pcall(function()
    game:BindToClose(onGameClosing)
end)

-- Também conectar quando o PlayerGui for limpo
